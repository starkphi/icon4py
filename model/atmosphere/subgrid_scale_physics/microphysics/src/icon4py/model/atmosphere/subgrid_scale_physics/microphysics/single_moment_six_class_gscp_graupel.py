...
class SingleMomentSixClassIconGraupel:
    ...
    def _initialize_configurable_parameters(self):
        ...
        _n0r: ta.wpfloat = (
            ta.wpfloat(8.0e6)
            * ta.wpfloat(math.exp(ta.wpfloat(3.2) * self.config.rain_mu))
        ...
        _ar: ta.wpfloat = (
            ...
            * PhysicsConstants.water_density
            / ta.wpfloat(6.0)
            * _n0r
            * ta.wpfloat(math.gamma(self.config.rain_mu + ta.wpfloat(4.0)))
        )  # pre-factor
        ...
        power_law_coeff_for_rain_mean_fall_speed: ta.wpfloat = (
            ta.wpfloat(130.0)
            * ta.wpfloat(math.gamma(self.config.rain_mu + ta.wpfloat(4.5)))
            / ta.wpfloat(math.gamma(self.config.rain_mu + ta.wpfloat(4.0)))
        ...
        precomputed_evaporation_alpha_coeff: ta.wpfloat = (
            ...
            * ta.wpfloat(math.gamma(self.config.rain_mu + ta.wpfloat(2.0)))
        )
        ...
        power_law_exponent_for_rain_mean_fall_speed_ln1o2: ta.wpfloat = ta.wpfloat(
            math.exp(power_law_exponent_for_rain_mean_fall_speed * ta.wpfloat(math.log(0.5)))
        )
        power_law_exponent_for_ice_mean_fall_speed_ln1o2: ta.wpfloat = ta.wpfloat(
            math.exp(
                MicrophysicsConstants.POWER_LAW_EXPONENT_FOR_ICE_MEAN_FALL_SPEED
                * ta.wpfloat(math.log(0.5))
            )
        )
        power_law_exponent_for_graupel_mean_fall_speed_ln1o2: ta.wpfloat = ta.wpfloat(
            math.exp(
